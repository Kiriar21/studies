...
class point{
	private:
		float v[3];
	public:
		point(){ for(int i=0;i<3;i++) v[i]=0; }
		point(float tab[]){
				for(int i=0;i<3;i++) 
					v[i]=tab[i];
		}
		point(const float a,const float b, const float c){ v[0]=a, v[1]=b, v[2]=c; }
		float& operator[](unsigned int n){ return v[n]; }
		const float &operator[](unsigned int n) const{ return v[n]; }
		const float distance(const point p1) const{	return sqrt((v[0] - p1[0]) * (v[0] - p1[0]) + (v[1] - p1[1]) * (v[1] - p1[1]) + (v[2] - p1[2]) * (v[2] - p1[2])); }
